...
function getUser(c) {
  ...
  ); /*gets channel on which to
    automatically move users from to their game */
  userId = chan.members.map(
    (user) => user.id
  ); /*parses the collection of members in the
    channel to get the id */
  userId = userId.toString(); //changes to id into a string
}
function getStatus(guildIdGetStatus) {
...
function moveUser(guildTBM) {
  ...
  memberTBM.voice.setChannel(
    client.channels.cache.find((channel) => channel.name == statusName)
  );
}
function main(b, d) {
  //starts process to move user
  getUser(b);
  getStatus(d);
}
